...
const checkProductExits: CheckProductCodeExists = (
  _: ProductCode.ProductCode
) => true;

const checkAddressExists: CheckAddressExists = (
  unvalidatedAddress: Inputs.UnvalidatedAddress
...
): Price.Price => Price.createUnsafe(10);

const getPromoitionPrices = (
  promotionCode: PromitionCode.PromitionCode
): TryGetProductPrice => {
  const halfPricePromotion: TryGetProductPrice = (
  ...
  const quarterPricePromotion: TryGetProductPrice = (
    productCode: ProductCode.ProductCode
  ) => {
    if (ProductCode.value(productCode) === "ONSALE") {
...
const getPricingFunction: GetPricingFunction = PricingModule.getPricingFunction(
  getStandardPrices
)(getPromoitionPrices);
const calculateShippingCost = Implementation.calculateShippingCost;

const createOrderAckLetter: CreateOrderAcknowledgmentLetter = (
  pricedOrder: PricedOrderWithShippingMethod
) => HtmlString.create("<h1>Hello World</h1>");
const sendOrderAck: SendOrderAcknowledgment = (
  _: OrderAcknowledgment
...
const workFlow = Implementation.placeOrder(checkProductExits)(
  checkAddressExists
)(getPricingFunction)(calculateShippingCost)(createOrderAckLetter)(
  sendOrderAck
);
const postOrder = async (req: Express.Request, res: Express.Response) => {
  const httpRes = await taskEither
    .fromEither(parseOrderDTO(req.body))
